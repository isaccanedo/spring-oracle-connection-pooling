...
class ImageIntegrationTest {
    ...
    @Test
    void givencanedoJpegImage_whenUploadIt_thenReturnItsId() throws Exception {
    ...
    void givenExistingImage_whenDownloadIt_thenReturnHttpStatusOk() throws Exception {
        given(imageRepository.findById(1L))
            .willReturn(Optional.of(canedoImage()));
    ...
    private MockMultipartHttpServletRequestBuilder createUploadImageRequest() throws IOException {
        ...
        return MockMvcRequestBuilders.multipart("/image")
            .file(new MockMultipartFile("multipartImage", "canedo", MediaType.TEXT_PLAIN_VALUE, image));
    }

    private Image canedoImage() throws IOException {
        ClassLoader classLoader = ClassLoader.getSystemClassLoader();

        Image mockImage = new Image();
        mockImage.setContent(Files.readAllBytes(Paths.get(classLoader.getResource("canedo.jpeg")
            .getFile())));
        return mockImage;
    }
